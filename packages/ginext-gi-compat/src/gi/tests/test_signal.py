...
class TestSignalModuleLevelFunctions(unittest.TestCase):
    ...
    def test_signal_query(self):
        ...
        query1 = GObject.signal_query("my-signal", C)
        assert query1 is not None
        self.assertEqual(list(query1), my_signal_expected_query_result)
        # signal_query(signal_id)
        query2 = GObject.signal_query(my_signal_id)
        assert query2 is not None
        self.assertEqual(list(query2), my_signal_expected_query_result)
...
class TestIntrospectedSignalsIssue158(unittest.TestCase):
    ...
    _obj_sig_names = [
        sig.get_name()
        for sig in cast(Any, repo.find_by_name("Regress", "TestObj")).get_signals()
    ]
...
class TestClosureRefCycle(unittest.TestCase):
    def test_closure_ref_cycle_unreachable(self):
        ...
        store_back = cast(Gio.ListStore[Any], x.get_attribute_object("store"))
        store_back.append(Gio.FileInfo())

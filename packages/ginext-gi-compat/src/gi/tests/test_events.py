...
class GLibEventLoopPolicyTests(unittest.TestCase):
    ...
    def test_glib_task_prio(self):
        ...
        async def run():
            t1 = cast(
                GLibTask[None],
                asyncio.create_task(run_prio(GLib.PRIORITY_DEFAULT_IDLE)),
            )
            t1.set_priority(GLib.PRIORITY_DEFAULT_IDLE)
            t2 = cast(
                GLibTask[None], asyncio.create_task(run_prio(GLib.PRIORITY_DEFAULT))
            )
            t2.set_priority(GLib.PRIORITY_DEFAULT)
            t3 = cast(GLibTask[None], asyncio.create_task(run_prio(GLib.PRIORITY_HIGH)))
    ...
    def test_asyncio_run(self):
        ...
        with self.assertRaises(KeyboardInterrupt):
            asyncio.run(
                cast(Coroutine[Any, Any, Any], LazyCoro()),
                loop_factory=gi.events.GLibEventLoop,
            )

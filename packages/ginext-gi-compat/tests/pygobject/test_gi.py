...
class TestFilename(unittest.TestCase):
    ...
    @pytest.mark.xfail(
        sys.platform == "win32",
        reason="ginext encodes Windows filenames with surrogatepass (handles "
        "lone surrogates) but does not yet combine surrogate PAIRS into a "
        "single codepoint (full WTF-8)",
        strict=True,
    )
...
class TestInterfaces(unittest.TestCase):
    ...
    def test_subclass_override(self):
        self.create_test_interface_impl()  # ensure class attribute is set

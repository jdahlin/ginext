...
try:
    ...
    Gtk = cast(Any, None)
    Gtk_version = None
    PyGTKDeprecationWarning = cast(Any, None)
    GdkPixbuf = cast(Any, None)
    Gdk = cast(Any, None)


@pytest.fixture(autouse=True, scope="module")
def _require_gtk_display(wayland):
    if Gtk is None:
        return
    ok = Gtk.init_check() if GTK4 else Gtk.init_check([])
...
def skipIfGtkVersionLessThan(version, reason):
    def decorator(func):
...
<interface>
  ...
  <object class="TestWindow" id="testpop">
    <property name="type">popup</property>
        ...
        self.assertEqual(win.get_property("type"), Gtk.WindowType.POPUP)
        testwin = builder.get_object("testwin")
        assert testwin is not None
        self.assertEqual(testwin.get_property("type"), Gtk.WindowType.TOPLEVEL)
        testpop = builder.get_object("testpop")
        assert testpop is not None
        self.assertEqual(testpop.get_property("type"), Gtk.WindowType.POPUP)
...
<interface>
  ...
  <object class="TestWindow" id="testwin">
    <property name="css-name">amazing-test</property>
        ...
        win = builder.get_object("win")
        assert win is not None
        self.assertEqual(win.get_property("css-name"), "amazing")
        testwin = builder.get_object("testwin")
        assert testwin is not None
        self.assertEqual(testwin.get_property("css-name"), "amazing-test")
...
class TestSignals(unittest.TestCase):
    @pytest.mark.xfail(
        reason="crashes in GTK class closure override with aliased type",
        run=False,
        strict=False,
    )
...
class TestBuilder(unittest.TestCase):
    ...
    def test_add_from_string(self):
        ...
        with pytest.raises(TypeError):
            builder.add_from_string(cast(str, object()))
  ...
  <menu id="appmenu">
    <section>
      <item>
        ...
        with pytest.raises(TypeError):
            builder.add_objects_from_string(cast(str, object()), [])
...
class TestTreeModelRow(unittest.TestCase):
    def test_tree_model_row(self):
        ...
        with pytest.raises(TypeError):
            Gtk.TreeModelRow(model, cast(Gtk.TreePath, 42))

        with pytest.raises(TypeError):
            Gtk.TreeModelRow(cast(Gtk.TreeModel, 42), path)
        ...
        assert prev[0] == 42
        prev2 = row.get_previous()
        assert prev2 is not None
        assert prev2[0] == 42
        prev3 = row.previous
        assert prev3 is not None
        assert prev3.previous is None
    ...
    def test_tree_model(self):
        ...
        for i, row in enumerate(tree_store):
            ...
            if i < len(tree_store) - 1:
                row_next = row.next
                assert row_next is not None
                self.assertEqual(tree_store.get_path(next), row_next.path)
                self.assertEqual(
            ...
            for j, childrow in enumerate(row.iterchildren()):
                ...
                childrow_parent = childrow.parent
                assert childrow_parent is not None
                self.assertEqual(childrow_parent.path, row.path)
                ...
                if j < 19:
                    childrow_next = childrow.next
                    assert childrow_next is not None
                    self.assertEqual(childrow_next.path, tree_store.get_path(child))
                else:
...
class TestTreeView(unittest.TestCase):
    ...
    def test_tree_view_add_column_with_attributes(self):
        ...
        with realized(tree):
            ...
            col0 = tree.get_column(0)
            col1 = tree.get_column(1)
            col2 = tree.get_column(2)
            col3 = tree.get_column(3)
            assert (
                col0 is not None
                and col1 is not None
                and col2 is not None
                and col3 is not None
            )
            self.assertEqual(col0.get_title(), "Head1")
            self.assertEqual(col1.get_title(), "Head2")
            self.assertEqual(col2.get_title(), "Head3")
            self.assertEqual(col3.get_title(), "Head4")
...
class TestTextBuffer(unittest.TestCase):
    def test_text_buffer(self):
        ...
        assert sel_bounds
        self.assertTrue(sel_bounds[0].equal(start))
        self.assertTrue(sel_bounds[1].equal(end))
    ...
    def test_insert(self):
        ...
        with pytest.raises(TypeError):
            buffer.insert(start, cast(str, 42))

        with pytest.raises(TypeError):
            buffer.insert_at_cursor(cast(str, 42))
    ...
    def test_text_buffer_search(self):
        ...
        result1 = i.forward_search("World", 0, None)
        assert result1 is not None
        (start, end) = result1
        self.assertEqual(start.get_offset(), 6)
        self.assertEqual(end.get_offset(), 11)

        result2 = i.forward_search("world", Gtk.TextSearchFlags.CASE_INSENSITIVE, None)
    ...
    @skipIfGtkVersionLessThan((3, 20, 0), "broken with older gtk")
    def test_backward_find_char(self):
        buffer = Gtk.TextBuffer()
        buffer.set_text("abc")
        res = buffer.get_iter_at_line(99)
        end: Gtk.TextIter = res.iter if GTK4 else cast(Gtk.TextIter, res)

        values = []

        def pred_func(ch: str, user_data: Any = None) -> bool:
            values.append(ch)
            return ch == "a"

...
class TestOverrides(unittest.TestCase):
    ...
    def test_load_no_overrides(self):
        ...
        try:
            ...
            sys.modules[mod_key] = cast(types.ModuleType, None)
            mod = gi.module.get_introspection_module("GIMarshallingTests")
            mod_override = gi.overrides.load_overrides(mod)
            ...
            if had_mod:
                sys.modules[mod_key] = cast(types.ModuleType, old_mod)
...
class TestImporter(unittest.TestCase):
    ...
    def test_require_version_versiontype(self):
        ...
        with self.assertRaises(ValueError):
            gi.require_version("GLib", cast(str, 2.0))

        with self.assertRaises(ValueError):
            gi.require_version("GLib", cast(str, b"2.0"))

...
GioUnix = None
try:
    from gi.repository import GioUnix  # type: ignore[no-redef]
except ImportError:
    pass
...
"""
    ...
    def assert_expected_desktop_app_info(self, app_info):
        ...
        with pytest.warns(DeprecationWarning, match="Gio.DesktopAppInfo is deprecated"):
            self.assertEqual(
...
class TestGApplication(unittest.TestCase):
    ...
    def test_local_command_line(self):
        ...
        self.assertEqual(res, 42)
        assert app.local_args is not None
    ...
    def test_add_action_entries_override(self):
        ...
        action = app.lookup_action("app.new")
        assert action is not None
        assert action.get_name() == "app.new"

...
# IMPORTANT: do NOT disable GTK auto-init at module-import time in the shared
# xdist worker. Collection imports every test module up front, so this module's
# `from gi.repository import Gtk` below would become the first Gtk access in the
# worker process; with auto-init disabled, gtk_init() is permanently skipped
# (the css overlay's first-access hook is sticky) and every later gtk4 widget
# test in that worker segfaults on its first g_object_new. Only disable auto-init
# inside the per-test subprocess (PYGIR_PYTEST_SUBPROCESS), where this module's
# signal/main-loop tests get the un-initialised GTK they were written against.
if os.environ.get("PYGIR_PYTEST_SUBPROCESS"):

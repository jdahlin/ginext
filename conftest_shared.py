...
def _restore_asan_preload() -> None:
    # macOS asan strips DYLD_INSERT_LIBRARIES from the environment after its
    # startup re-exec, so any child this process spawns (subprocess markers,
    # probe runners, inline subprocess.run in tests) would load the asan-
    # instrumented extension too late ("interceptors are not working").
    # Restore it into os.environ from the path the Makefile stashed in a var
    # asan leaves alone, so every spawned child inherits the preload.
...
def suppress_editable_rebuild() -> tuple[list[str] | None, str | None]:
    # meson-python's editable import hook rebuilds via meson. Tests already run
    # against a configured builddir, so suppress that hook and rebuild once with
    # the recorded ninja command instead.
    build_path = None
    ninja_cmd = None
    for finder in sys.meta_path:
        if type(finder).__name__ != "MesonpyMetaFinder":
            continue
        # Only the core ginext finder is suppressed/rebuilt here. Compiled
        # overlay-package finders (e.g. ginext_gst) stay active so they keep
        # serving their own extension module.
...
def rebuild_editable(
    ...
    # ninja_cmd/build_path identify the (suppressed) core finder, but we rebuild
    # *every* meson-python editable here so compiled overlay packages (ginext_gst,
    # whose finder stays active and rebuilds on import) are current before workers
    # spawn — otherwise the workers race on a concurrent ninja in one build dir.
...
def setup_package_paths(root: pathlib.Path) -> None:
...
def register_win_dll_dirs() -> None:
    """Register the Windows DLL search dirs (GINEXT_WIN_DLL_DIRS) so the C
    extension and its vcpkg deps load. On Windows DLL deps of a .pyd are not
    found via PATH, only via os.add_dll_directory. The repo-root conftest does
    this for the main run, but subprocess test runs use the package as rootdir
    (so that conftest is not loaded) and reach ginext through here instead."""
...
def setup_gi_test_env(root: pathlib.Path, build_path: str | None = None) -> None:
    ...
    # A build dir on sys.path (e.g. PYTHONPATH=build/<py>/src) carries the
    # matching test libraries; prefer it over an arbitrary build/* scan.

...
try:
    from gi.repository import Gio, Gdk, GdkPixbuf
...
def skipUnlessGtkVersionAtLeast(version, reason):
    def decorator(func):
...
class TestGdk(unittest.TestCase):
    ...
    def test_rgba_gtk4(self):
        with pytest.warns(DeprecationWarning, match="RGBA positional"):
            c = Gdk.RGBA(0, 0, 0, 0)
    ...
    @unittest.skipUnless(GDK4, "only in gdk4")
    @skipUnlessGtkVersionAtLeast((4, 8, 0), "constructor available since 4.8")
    ...
    @unittest.skipUnless(GDK4, "only in gdk4")
    @skipUnlessGtkVersionAtLeast((4, 21, 0), "available since 4.21")

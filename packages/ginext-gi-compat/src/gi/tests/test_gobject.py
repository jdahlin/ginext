...
class TestGObjectAPI(unittest.TestCase):
    ...
    def test_min_max_int(self):
        ...
        self.assertTrue(
            all(issubclass(warning.category, PyGIDeprecationWarning) for warning in w)
        )
        ...
        for name in (
            ...
            self.assertIn(
                f"GObject.{name} is deprecated; use GLib.{name[2:]} instead",
                messages,
            )
...
class TestDispose(unittest.TestCase):
    ...
    def test_only_call_do_dispose_if_defined_on_class(self):
        ...
        class TestObject(GObject.Object):
            def __init__(self):
                setattr(self, "do_dispose", do_dispose)
                super().__init__()

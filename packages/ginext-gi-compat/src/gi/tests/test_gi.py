    ...
    def test_path_exists_various_types(self):
        ...
        for d, path in paths:
            if not valid_path(path):
                continue
            path = os.path.join(cast(Any, d), path)
            with open(path, "wb"):
                self.assertTrue(GIMarshallingTests.filename_exists(path))
...
class TestArray(unittest.TestCase):
    ...
    def test_array_fixed_caller_allocated_struct_out_item_lifetime(self):
        array = GIMarshallingTests.array_fixed_caller_allocated_struct_out()
        item = array[0]
        ...
        self.assertIsInstance(item, GIMarshallingTests.SimpleStruct)
        self.assertEqual(item.long_, -2)
        self.assertEqual(item.int8, -1)
...
class TestGHashTable(unittest.TestCase):
    ...
    def test_ghashtable_int64_in(self):
        GIMarshallingTests.ghashtable_int64_in(
            {"-1": -1, "0": 0, "1": 1, "2": GLib.MAXUINT32 + 1}
        )
...
class TestGObject(unittest.TestCase):
    ...
    def test_constructor_bad_cls_arg(self):
        ...
        newv: Any = GObject.GObject.newv.__func__
...
class TestInterfaces(unittest.TestCase):
    TestInterfaceImpl: type[Any]

    def create_test_interface_impl(self):
        ...
        type(self).TestInterfaceImpl = TestInterfaceImpl
    ...
    def test_implementation(self):
        TestInterfaceImpl = self.create_test_interface_impl()
...
class TestMRO(unittest.TestCase):
    ...
    def test_interface_collision(self):
        ...
        # type() used to avoid static MRO check; runtime handles GObject diamond inheritance
        TestInterfaceImpl2 = type(
            "TestInterfaceImpl2", (GIMarshallingTests.Interface, TestInterfaceImpl), {}
        )
...
class TestKeywords(unittest.TestCase):
    ...
    def test_uppercase(self):
        ...
        self.assertEqual(cast(_GObject.GFlags, GLib.IOCondition.IN).value_nicks, ["in"])
...
class TestDeprecation(unittest.TestCase):
    ...
    def test_deprecated_attribute_compat(self):
        ...
        try:
            ...
            setattr(GLib, "IO_STATUS_ERROR", "foo")

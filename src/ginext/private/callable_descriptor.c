...
PyObject *
py_invoke_class_struct_callable (PyObject *module G_GNUC_UNUSED, PyObject *args)
{
  PyObject *cls = NULL;
  PyObject *struct_cls = NULL;
  PyObject *descriptor_capsule = NULL;
  ...
  PyGIMethodDescriptor *descriptor = method_descriptor_from_py (descriptor_capsule);
  if (descriptor == NULL)
    return NULL;
  if (!PyTuple_Check (py_args))
    {
  ...
  PyObject *kwargs = py_kwargs;
  if (kwargs == Py_None)
    kwargs = NULL;
  else if (kwargs != NULL && !PyDict_Check (kwargs))
    {
      PyErr_SetString (PyExc_TypeError, "kwargs must be a dict or None");
      return NULL;
    }

  PyObject *self_obj = class_struct_wrapper_for_type (cls, struct_cls);
  if (self_obj == NULL)
    return NULL;

  PyObject *final_args = resolve_call_args (descriptor, self_obj, py_args, 0, kwargs);
  Py_DECREF (self_obj);
  if (final_args == NULL)
    return NULL;

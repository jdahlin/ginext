...
class _GLibEventLoopMixin:
    ...
    if TYPE_CHECKING:
        _selector: Any
        _debug: bool
        _asyncgen_firstiter_hook: Any
        _asyncgen_finalizer_hook: Any
        _ready: list[Any]
        _scheduled: list[Any]
    ...
    def running(self, quit_func):
        ...
        if hasattr(self, "_run_forever_setup"):
            ...
            asyncio._set_running_loop(cast("asyncio.AbstractEventLoop", self))
    ...
    def _call_soon(self, callback: Any, args: Any, context: Any) -> Any:
        try:
        ...
        if priority == GLib.PRIORITY_DEFAULT:
            ...
            return cast("Any", super())._call_soon(callback, args, context)

        handle = asyncio.Handle(
            callback, args, cast("asyncio.AbstractEventLoop", self), context
        )
...
class _GLibEventLoopRunMixin:
    ...
    if TYPE_CHECKING:
        ...
        def is_running(self) -> bool: ...
        def running(self, quit_func: Any) -> Any: ...
...
class _SelectorMixin:
    ...
    def close(self):
        ...
        if self._source and hash(self._source):
            ...
            self._source = None
        cast("selectors.BaseSelector", super()).close()
    ...
    def _real_select(self, timeout=None):
        return cast("selectors.BaseSelector", super()).select(timeout)
...
if sys.platform != "win32":
    ...
    class GLibEventLoop(
        ...
        def add_signal_handler(self, sig, callback, *args):
            ...
            if coroutines.iscoroutine(callback) or inspect.iscoroutinefunction(
                callback
            ):
            ...
            source = GLibUnix.signal_source_new(sig)
        ...
        def remove_signal_handler(self, sig):
            ...
            try:
                ...
                # Destroy the GSource so that GLib resets the signal handler.
                source.destroy()
        ...
        def _signal_cb(self, sig):
            _source, cb, args = self.__signal_handlers[sig]
    ...
    class _Source(_SourceBase):
        ...
        def check(self):
            ...
            sel = self._selector()
            assert sel is not None
            for key in sel._fd_to_key.values():
...
_ASYNCIO_POLICY_DEPRECATION_RE = (
    r"'asyncio\.(AbstractEventLoopPolicy|get_event_loop_policy|"
    r"set_event_loop_policy)' is deprecated and slated for removal in Python 3\.16"
)


@contextmanager
def _ignore_asyncio_policy_deprecation():
    # These policy APIs are intentionally used for compatibility until they
    # disappear in Python 3.16.
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
            message=_ASYNCIO_POLICY_DEPRECATION_RE,
...
if TYPE_CHECKING:
    AbstractEventLoopPolicy: type = asyncio.AbstractEventLoopPolicy  # type: ignore[attr-defined]
else:
    with _ignore_asyncio_policy_deprecation():
        AbstractEventLoopPolicy = getattr(asyncio, "AbstractEventLoopPolicy", object)

_set_asyncio_event_loop_policy = getattr(asyncio, "set_event_loop_policy", None)
_get_asyncio_event_loop_policy = getattr(asyncio, "get_event_loop_policy", None)


def _set_event_loop_policy(policy):
    if _set_asyncio_event_loop_policy is None:
        return None
    with _ignore_asyncio_policy_deprecation():
        return _set_asyncio_event_loop_policy(policy)


def _get_event_loop_policy():
    if _get_asyncio_event_loop_policy is None:
        return None
    with _ignore_asyncio_policy_deprecation():

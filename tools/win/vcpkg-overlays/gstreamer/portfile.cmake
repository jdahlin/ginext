...
# With introspection enabled, gstreamer's docs/meson.build references
# gst_check_gir, which isn't built (the check library is disabled). Drop those
# refs so configure succeeds (docs are off anyway).
vcpkg_replace_string(
    "${SOURCE_PATH}/subprojects/gstreamer/docs/meson.build"
...
# Introspection: generate the gst typelibs (Gst-1.0, GstBase, GstApp, GstAudio,
# GstVideo, GstPbutils, ...). Reuse gobject-introspection's port-config for a
# cp3xx scanner venv (its _giscanner is cp3xx; the build python may differ) plus
# the resolved g-ir-scanner/g-ir-compiler, and wire the VCPKG_GI_* scan env and
# pkg-config the scanner shells out to (mirrors the glib-gir overlay).
include("${CURRENT_HOST_INSTALLED_DIR}/share/gobject-introspection/vcpkg-port-config.cmake")
vcpkg_get_gobject_introspection_programs(PYTHON3 GIR_COMPILER GIR_SCANNER)
set(GIR_SCANNER_PY "${PYTHON3}")
# The port-config sets VIRTUAL_ENV so meson would pick the scanner venv as the
# build's python. gstreamer's nested `meson --internal exe` steps (glib-mkenums)
# crash under it; keep the build on the normal vcpkg python3 and use the venv
# only for g-ir-scanner (passed explicitly above).
...
vcpkg_add_to_path(PREPEND "${CURRENT_HOST_INSTALLED_DIR}/tools/gobject-introspection")
...
vcpkg_configure_meson(
    ...
    ADDITIONAL_BINARIES
        g-ir-scanner=['${GIR_SCANNER_PY}','${GIR_SCANNER}']

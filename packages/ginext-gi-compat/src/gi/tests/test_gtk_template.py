...
def new_gtype_name(_count=[0]):
    # Probe the process-global GType registry so the name is unique no matter
    # which other test module (using this same scheme) already claimed numbers
    # in this worker — PyGObject-style "a base name plus a number if needed".
    while True:
        _count[0] += 1
        name = f"GtkTemplateTest{_count[0]:d}"
        try:
